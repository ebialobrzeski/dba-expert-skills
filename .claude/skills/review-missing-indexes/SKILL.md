...
Load `dba-standard` alongside this skill. Every recommendation follows its P0.2 (rollback scripts),
P0.3 (risk level), P1.6 (write impact), P2.3 (edition awareness) and P2.5 (consolidation) rules.
...
Follow the `dba-standard` output template, with these specifics:
...
- Related diagnostics: `queries/sqlserver/expensive-queries.sql`, `review-index-usage` skill
